...
it('publishes the stancl tenancy foundation for host applications', function (): void {
    ...
    expect($readme)->toContain('Read-only split repository')
        ...
        ->and($tenantRouteFile)->toContain('RedirectImpersonatingTenantGuest::class,')
        ->and($tenantRouteFile)->toContain('$packageTenancyRoutesRoot = base_path(\'vendor/mapo-89/core-panel-tenancy/routes/web\');')
        ->and($tenantRouteFile)->toContain('Route::middleware($tenantWebMiddleware)->group(function () use ($corePanelRouteMiddleware, $loadTenantWebRouteFile, $webRoutes, $packageTenancyRoutesRoot): void {')
        ->and($tenantRouteFile)->toContain("Route::name('tenant.')->group(function () use (\$corePanelRouteMiddleware, \$loadTenantWebRouteFile, \$webRoutes, \$packageTenancyRoutesRoot): void {")
        ->and($tenantRouteFile)->toContain('])->group(function () use ($loadTenantWebRouteFile, $webRoutes, $packageTenancyRoutesRoot): void {')
        ->and($tenantRouteFile)->toContain('$hostTenantRoutesPath = base_path(\'routes/web/tenants.php\');')
        ->and($tenantRouteFile)->toContain('$packageTenantRoutesPath = $packageTenancyRoutesRoot.\'/tenants.php\';')
        ...
        ->and($tenantUsersMigration)->toContain("\$table->timestamp('invited_at')->nullable();")
        ->and($tenantUsersMigration)->toContain("\$table->timestamp('invitation_accepted_at')->nullable();")
